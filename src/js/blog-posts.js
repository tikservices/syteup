...
function adjustBlogHeaders() {
    ...
    $(".blog-section article hgroup").each(function (i, e) {
        $(e).find("h3 a").css({ "margin-top": "-" + ($(e).height() + 100) + "px" }).addClass("adjusted");
    });
}
function setupBlogHeaderScroll() {
    if (window.isMobileView)
        return;
    var previousTarget, activeTarget, $window = $(window), offsets = [], targets = [], $posts = $(".blog-section article hgroup h3 a").each(function () {
            if (this.hash) {
    ...
    function processScroll(e) {
        var scrollTop = $window.scrollTop(), i = offsets.length;
        for (i; i--;) {
