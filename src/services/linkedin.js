(function(window) {
    ...
    function fetchData(settings) {
        ...
        return Promise.all([
            asyncGet(API_URL + "/people::(" + settings.username + "):(" + profile_selectors + ")", {
                oauth_token: settings.token
            }),
            asyncGet(API_URL + "/people/id=" + settings.username + "/group-memberships:(group:(id,name),membership-state)", {
                oauth_token: settings.token
            }),
            asyncGet(API_URL + "/people/id=" + settings.username + "/network/updates?type=" + network_upd_types, {
                oauth_token: settings.token
            })
        ]).then(function(res) {
    ...
    window.linkedinService = {
        ...
        template: "templates/linkedin-view.html",
        setup: setupLinkedin,
        fetch: fetchData
    };
})(window);
